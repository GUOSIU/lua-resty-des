...
function _M.encrypt_pkcs(key, str)
	...
	local gen_key = ffi_new("unsigned char[?]", 24)
	local des     = C.EVP_des_ede3_ecb()
...
function _M.decrypt_vb6(key, str)
	...
	local padding_size = 0
		while buf[str_len - padding_size - 1] == 0 and
			  buf[str_len - padding_size - 2] == 0 do
			padding_size = padding_size + 2 -- 双字节
		end
	return ffi_str(buf, out_len[0]-padding_size)
end
